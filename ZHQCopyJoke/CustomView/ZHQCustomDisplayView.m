...
@interface ZHQCustomDisplayView ()<CALayerDelegate>

@property (assign,nonatomic)CGRect          oriFrame;
@property (strong,nonatomic)ZHQAVPlayerManager     * vnaPlayer;
/** 上部视图 */
...
@end
@implementation ZHQCustomDisplayView
...
- (instancetype)initWithFrame:(CGRect)frame andvideoURLstr:(NSString *)videoURLstr
{
    if (self = [super initWithFrame:frame]) {
        if (videoURLstr != nil) {
            _oriFrame = frame;
            ZHQAVPlayerManager * playV = [[ZHQAVPlayerManager alloc]initWithFrame:frame withUrl:videoURLstr];
...
-(void)fullscreenAction:(UIButton *)fullBtn
{
    ...
    if (!_isFullScreen) {//如果不是全屏
      ...
      [[UIDevice currentDevice]setValue:[NSNumber numberWithInteger:UIDeviceOrientationPortrait]  forKey:@"orientation"];//        self.frame = _oriFrame;
        self.vnaPlayer.frame = _oriFrame;
        [self updateConstraintsIfNeeded];
    }else{
        self.isFullScreen = NO;
[[UIDevice currentDevice]setValue:[NSNumber numberWithInteger:UIDeviceOrientationLandscapeLeft]  forKey:@"orientation"];//        self.vnaPlayer.frame = self.window.bounds;
